        ...
        switch (focusChange) {
            ...
            case AudioManager.AUDIOFOCUS_GAIN:
                if (isPausedByFocusLossTransient) {
                    // 通话结束，恢复播放
                    AudioPlayerUtil.get().startPlayer();
                }

                // 恢复音量
                AudioPlayerUtil.get().getMediaPlayer().setVolume(1f, 1f);
            ...
            case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                AudioPlayerUtil.get().pausePlayer(false);
                isPausedByFocusLossTransient = true;
                break;
            // 瞬间丢失焦点，如通知
            case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK:
                // 音量减小为一半
                AudioPlayerUtil.get().getMediaPlayer().setVolume(0.5f, 0.5f);
                break;
        }
    }
}
